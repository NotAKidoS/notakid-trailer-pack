local light_table = {
	...
	ems_sounds = { "common/null.wav", "gtavredux/REVERSE_SOUND.wav" },
	ems_sprites = {
		{pos = Vector(16,45.5,75),material = "sprites/light_glow02_add_noz",size = 40,Colors = {Color(50,0,40,50),Color(100,50,0,100),Color(150,60,0,150),Color(200,70,0,200),Color(250,80,0,250),Color(255,85,0,255),Color(250,80,0,250),Color(200,70,0,200),Color(150,60,0,150),Color(100,50,0,100),Color(50,40,0,50),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0)},Speed = 0.01},
		{pos = Vector(16,45.5,75),material = "sprites/light_glow02_add_noz",size = 40,Colors = {Color(50,0,40,50),Color(100,50,0,100),Color(150,60,0,150),Color(200,70,0,200),Color(250,80,0,250),Color(255,85,0,255),Color(250,80,0,250),Color(200,70,0,200),Color(0,60,0,150),Color(0,50,0,100),Color(0,40,0,50),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0),Color(0,0,0,0)},Speed = 0.01},
...
list.Set( "simfphys_lights", "Towtruck_lights", light_table)



if SERVER then
	hook.Add("Think", "naktowtruck______", function() 
		for k,ent in pairs(ents:GetAll()) do
			if ( ent:GetClass() == "gmod_sent_vehicle_fphysics_base" ) then
				if ent:GetNWBool("naktowtruck________") then
					if ent:GetNWBool("towinstallized") then
								
					ent.towhook = ents.Create("prop_physics")
					ent.towhook:SetModel("models/notakid/gtavredux/towtruck/towhook.mdl")
					ent.towhook:SetPos(ent:GetPos() + ent:GetForward() * -180 + ent:GetRight() * 0 + ent:GetUp() * 80 )
					ent.towhook:SetAngles(Angle(spikeangle),Angle(spikeangle),Angle(spikeangle))
					ent.towhook:Spawn()
					ent.towhook:GetPhysicsObject():SetMass(10)
					ent.towhook:Activate()
					ent.towhook:PhysicsInit(SOLID_VPHYSICS)
					ent.towhook:SetMoveType(MOVETYPE_VPHYSICS)
					ent.towhook:SetSolid(SOLID_VPHYSICS)

					constraint.Rope( ent, ent.towhook, 0, 0, Vector(-162.141,-5.8,74), Vector(0,-0.01,0), 40, 0, 0, 1.5, "models/notakid/gtavredux/towtruck_rear", false )			
					constraint.Rope( ent, ent.towhook, 0, 0, Vector(-162.141,5.8,74), Vector(0,0.01,0), 40, 0, 0, 1.5, "models/notakid/gtavredux/towtruck_rear", false )	
					
					ent:SetNWBool("towinstallized", false)
...
local V = {
	Name = "Towtruck",
	Model = "models/notakid/gtavredux/Towtruck/Towtruck_main.mdl",
	...
	Members = {
		...
		OnSpawn = function(ent)
			...
			ent:SetNWBool("naktowtruck________", true)
			ent:SetNWBool("towinstallized", true)
			
        end,
		OnTick = function(ent)
			ent:CallOnRemove( "towtrucktowhookremove", function( ent ) if ent.towhook:IsValid() then ent.towhook:Remove() end end )
		end,
			
		
		LightsTable = "Towtruck_lights",
		
		CustomWheels = true,
		CustomSuspensionTravel = 0,
		StrengthenSuspension = true,
		
		CustomWheelModel = "models/notakid/gtavredux/Towtruck/Towtruck_f_wheel.mdl",
		CustomWheelModel_R = "models/notakid/gtavredux/Towtruck/Towtruck_r_wheel.mdl",
...
list.Set( "simfphys_vehicles", "sim_fphys_gtav_Towtruckreduxextra", V )